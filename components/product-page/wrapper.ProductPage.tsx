...
export default function PageProduct({item} : {item : {slug : string, category : string}}) {
  ...
  function detailReducer(state : initTransactionType, action : actionTrdType) : initTransactionType{
    switch(action.key){
      case "ADD_ID" : return {...state,id : action.payload}
      case "ADD_BRAND" : return {...state,brand : action.payload}
      case "ADD_PRODUCT" : return {...state,product : action.payload}
      case "ADD_PRICE" : return {...state,price : action.payload}
      case "ADD_USERID" : return {...state,userId : action.payload}
      case "ADD_SERVERID" : return {...state,serverId : action.payload}
      case "ADD_PAYMENT" : return {...state,payment : action.payload}
      case "ADD_VOUCHER" : return {...state,voucher : action.payload}
  ...
  useEffect(()=>{
    const aData = indeticalData.find((data)=>data.id === wrapDetailTrd.detailTrd.id)
    if(aData){
      wrapDetailTrd.setDetailTrd({
        key : "ADD_PRICE",
        payload : aData.price
  ...
  return (
    <div className="grid gap-5 mx-5 h-100 grid-cols-1 md:grid-cols-5 relative pb-10">
      <DesProductPage brandInfo={wrapDetailTrd.detailTrd.brand} />
      <ContProductPage datas={indeticalData} setModal={handleModal} wrapDetailTrd={wrapDetailTrd} />
